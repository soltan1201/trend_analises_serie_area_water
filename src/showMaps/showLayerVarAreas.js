...
Map.addLayer(regions , {color: '#A7E6FF'}, 'regions');
var codeCountry = '4';
var imColArea = ee.ImageCollection(param.asset_input).filter(
...
lstMeses.forEach(function(mes){
    ...
    var watertmp = layerWater.filter(ee.Filter.eq('cadence', 'monthly'))
                            .filter(ee.Filter.eq('year', nyear));    
    var imagename = String(nyear) + "_mes_" + String(month + 1);
    var bandaAct = 'classification_' + String(month + 1);
