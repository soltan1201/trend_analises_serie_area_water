...
params = {
    ...
    'asset_layerArea': 'projects/nexgenmap/GTAGUA/grade_area_to_imColrbr',
    'version': 11,
...
siglaCount = 'bra'
lstlayerAreas = ee.ImageCollection(params['asset_layerArea'])
# lstAssetIdAreasLayer = lstlayerAreas.reduceColumns(ee.Reducer.toList(), ['system:index']).get('list')
# print(f" we loaded  {len(getlstGridAreas)} features grades, we filtered by {siglaCount}")
# print("the first ", getlstGridAreas[0])

dictReg = {}
for jj, idcode in enumerate(lstreg):        
    print(f" ======= loading region {idcode}  ======")
    dictregLayers = {}

    for yyear in range(1985, 2024):
        regLayersYY = lstlayerAreas.filter(ee.Filter.eq('code_region', idcode)).filter(
                                        ee.Filter.eq('year', yyear))
        
        dictregLayers[str(yyear)] = regLayersYY.size().getInfo()
    
    dictReg[idcode] = dictregLayers
    
for kkey, dictYY in dictReg.items():
    print(f"showing regions {kkey}  ===")
    lstFaltam = []
    lstValY = [yy for yy in dictYY.keys()]
    for yyear in range(1985, 2024):
        if str(yyear) not in lstValY:
            lstFaltam.append(str(yyear))
        else:
            for ykey, val in dictYY.items():
                # print(ykey, val)
                if val != 12:
                    lstFaltam.append(str(yyear))
